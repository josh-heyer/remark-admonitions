...
function attacher(options) {
  ...
  function attachParser(parser) {
    if (!parser || !parser.prototype || !parser.prototype.blockTokenizers || !parser.prototype.blockMethods)
      return;
  ...
  function addEscapes() {
    // add the start of the tag as a valid escape
    if (pipeline.escapes)
      pipeline.escapes.push(options.tag[0]);
    if (pipeline.escape) {
      const oldEscape = pipeline.escape;
      pipeline.escape = function (value, node, parent) {
        value = value.replace(regex, "\$&");
        return oldEscape.call(pipeline, value, node, parent);
      }
    }
  }
  ...
  function attachCompiler(compiler) {
    ...
    compiler.prototype.visitors["admonition"] = function (node) {
      ...
      if (heading.children.length && heading.children[0].data
        && heading.children[0].children
        && heading.children[0].data.hName === "h5") {
        heading.children = heading.children[0].children.filter(n => n.type !== "admonition-icon");
      }
      ...
      if (content && node.indentSize)
        content = content.split(NEWLINE).map(l => l ? " ".repeat(node.indentSize) + l : l).join(NEWLINE);

      if (heading || node.quoted) {
        const quote = node.quoted
          ? (heading.includes('"') ? "'" : '"')
          : '';
      ...
      let keywordSpace = "";
      if (node.indentSize > tag.length)
        keywordSpace = " ".repeat(node.indentSize - tag.length);
      else if (!content)
        keywordSpace = " ";

      return tag + keywordSpace + node.keyword + heading + NEWLINE
        + (content ? content : '')
        + (!node.indentSize && content ? NEWLINE + tag : '');
    };
  }
  ...
  function hastFriendlyTransformer(tree) {
    visit(tree, "admonition", node => {
      ...
      const iconContainerNodes =
        config.icons === "none"
          ? heading.children
          : [element("span", ["admonition-icon"], [iconNodes]), ...heading.children];

      // rebuild heading with HAST-friendly attributes
      heading = element("div", ["admonition-heading"], [
